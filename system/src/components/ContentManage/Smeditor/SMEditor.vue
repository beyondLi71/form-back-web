  ...
  <div class="smeditor" id="smeditor">
    <div class="buttons" :class="buttonsBarFixed == true ? 'isFixed' :''">
      ...
      <button type="button" v-for='(name, index) in basicIcons'
              ...
              v-on:mouseover.stop='mouseover($event)'
              v-bind:title='basicStyleNames[index]'>
        <img :src='iconSrc(name)'></img>
      </button>
      <button type="button" class='set-font' @click.stop="titleButtonClick" title="标题">
        <span>H</span>
      ...
      <button type="button" class='insert-options' @click="isInsertShow = !isInsertShow">
        ...
        <insert-options v-show="isInsertShow"
                        ...
                        :uploadImages='uploadImages'></insert-options>
      </button>
...
export default {
  ...
  data () {
    return {
      ...
      // basicStyleNames: ['粗体', '下划线', '斜体', '中划线'],
      basicIcons: ['bold', 'italic'],
      basicStyleNames: ['粗体', '斜体'],
      // 调色盘是否显示
